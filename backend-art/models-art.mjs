...
db.once("open", (err) => {
    if(err){
        res.status(500).json({ error: 'Connection to the Arts database collection on the MongoDB Atlas Cluster failed.' });
    } else  {
        console.log('Connection to the Arts Collection on the MongoDB Atlas Cluster was successful.');
    }
});
